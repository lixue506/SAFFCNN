...
class cDt():
    ...
    def Ynp(self, n):
        ...
        for k in range(self.m):
            if k > 0:
                K = K * k
            result += self.fai( k*self.h ) * pow(Tnk, k) / K
        ...
        for j in range(n+1):
            ...
            t += self.bjn(n+1, j) * self.f(tj, yj, vj, j)
    ...
    def Vn(self, n):
        ...
        if self.m == 1:
            ...
            result = self.deerta * self.yn[n-self.m+2] + (1 - self.deerta) * self.yn[n-m+1]
        print("  VN:  ", result)
        self.vn[n+1] = result
    ...
    def Yn(self, n):
        ...
        result += pow(self.h, self.alpha) / gamma(self.alpha+2) * self.f(tn, y, v, n+1)
        ...
        for j in range(n+1):
            ...
            t += self.ajn(n, j) * self.f(tj, yj, vj, j)
    ...
    # 绘图     
    # def Show(self):
    #     xt = [math.sin(i*self.h) for i in range(self.N+1)]
    #     t = [math.cos(i*self.h) for i in range(self.N+1)]
    #     plt.title('alpha = 0.725')
    #     plt.xlabel('t')
    #     plt.ylabel('xt')
    #     plt.plot(t, xt) # 绘制曲线 y1
...
if __name__ == "__main__":
    ...
    plt.xlabel('xt')
    plt.ylabel('yt')
    plt.plot(xt, yt[0:len(xt)]) # 绘制曲线 y1
    # plt.savefig('./xt_t2.jpg')
    plt.show()
